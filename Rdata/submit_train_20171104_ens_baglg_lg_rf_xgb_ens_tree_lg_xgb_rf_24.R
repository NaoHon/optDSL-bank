...
for(i in 1:L){
  ...
  #検証データへ当てはめ
  pred_logi_train_test<-predict(logi_model, newdata=dplyr::select(train_test, -id, -y), type="response")
  #AUCの計算
  #roc(目的変数(1 or 0), 予測結果)
  auc_tmp<-roc(train_test$y, pred_logi_train_test)
  auc <- c(auc, as.numeric(auc_tmp$auc))
  
  #予測結果をとっておく
  {
    if(i==1){score_tmp<-pred_logi_train_test}
    else{score_tmp<-data.frame(score_tmp, pred_logi_train_test)}
  }
...
bag_pred_logi_train_test<-apply(score_tmp, 1, mean)

#AUCの計算
#roc(目的変数(1 or 0), 予測結果)
auc_tmp<-roc(train_test$y, bag_pred_logi_train_test)
auc <- c(auc, as.numeric(auc_tmp$auc))
...
#予測結果の平均
#apply(データ, 1, 関数)でデータを行ごとに横ベクトルとして関数に適用
bag_pred_logi<-apply(score_tmp, 1, mean)
...
rf<-randomForest(rf_train_train, #学習データ(説明変数)
                 ...
                 nodesize=75, #生成する各決定木のノードが含むサンプル最小数
                 maxnodes=50, #生成する各決定木の終端ノードの最大数
...
for(j in 1:K){
  ...
  #sample(ベクトル, ランダムに取得する個数, 復元抽出の有無)
  row<-sample(1:nrow(train_tmp), num, replace=FALSE)
  
  #構築データ
  rf_train_train_tmp<-train_tmp[row,] %>%
    dplyr::select(-id, -y)
  
  #検証データ
  rf_train_test_tmp<-train_tmp[-row,] %>%
    dplyr::select(-id, -y)
  
  #目的変数作成
  y_train_train_tmp<- train_tmp[row,] %>%
    dplyr::select(y)
  y_train_test_tmp<- train_tmp[-row,] %>%
    dplyr::select(y)
  ...
  #チューニング
  #tuneRF(rf_train_train_tmp, as.factor(y_train_train_tmp$y),doBest=TRUE)
  
  rf_tmp<-randomForest(rf_train_train_tmp, #学習データ(説明変数)
                   as.factor(y_train_train_tmp$y), #学習データ(目的変数)
                   mtry=2, #1本の木に使用する変数の数
                   sampsize=nrow(y_train_train_tmp)*0.3, #モデル構築に使用するデータ数
                   nodesize=75, #生成する各決定木のノードが含むサンプル最小数
  ...
  ##train_testのAUC
  #prediction(予測結果,目的変数(1 or 0))
  rf_pred_train_test_tmp <-predict(rf_tmp, newdata=rf_train_test_tmp, type="prob")[,2]
  auc<-roc(y_train_test_tmp$y, rf_pred_train_test_tmp)
  print(auc)
  ...
  pred_train_rf<-predict(rf_tmp, newdata=test_tmp, type="prob")[,2]
  y<-c(y, test_tmp$y)
  ...
  score_train_rf<-c(score_train_rf, pred_train_rf)
  ...
  pred_test_rf<-predict(rf_tmp, train_test)
  ...
  score_test_rf<-cbind(score_test_rf, pred_test_rf) 
}
...
score_test_rf<-apply(score_test_rf, 1, mean)
m_dat_test1<-data.frame(tree=score_test_tree, logi=score_test_logi,xgb=score_test_xgb,rf=score_test_rf,y=train_test$y)


#メタモデル用変数作成
m_dat_train<-data.frame(tree=score_train_tree, logi=score_train_logi,xgb=score_train_xgb,rf=score_train_rf, y=y)
...
for(j in 1:K){
  ...
  pred_test_xgb<-predict(model_xgb_tmp, x_test)
  pred_test_rf<-predict(rf_tmp, test)
...
score_test_xgb<-apply(score_test_xgb, 1, mean)
score_test_rf<-apply(score_test_rf, 1, mean)
m_dat_test1<-data.frame(tree=score_test_tree, logi=score_test_logi,xgb=score_test_xgb,rf=score_test_rf)
...
#データ加工
out<-data.frame(test$id, (0.3*bag_pred_logi+2.9*pred_rf+2.9*pred_xgb+3.9*stack_pred_test)/10)

write.table(out,
            file="C:/study/bank/submit/submit_train_20171104_ens_baglg_lg_rf_xgb_ens_tree_lg_xgb_rf_24.csv",
            quote=F, sep=",", row.names=F, col.names=F)
