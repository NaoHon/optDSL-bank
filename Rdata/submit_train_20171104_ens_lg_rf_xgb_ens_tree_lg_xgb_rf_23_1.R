...
m_dat_test1<-data.frame(tree=score_test_tree, logi=score_test_logi,rf=score_test_rf)
#メタモデル用変数作成
# m_dat_train<-data.frame(tree=score_train_tree, logi=score_train_logi,xgb=score_train_xgb,rf=score_train_rf, y=y)
m_dat_train<-data.frame(tree=score_train_tree, logi=score_train_logi,rf=score_train_rf, y=y)
...
#データ加工
out<-data.frame(test$id, (3*pred_rf+3*pred_xgb+4*stack_pred_test)/10)
#out<-data.frame(test$id, (3*pred_rf+3.6*pred_xgb+3.4*stack_pred_test)/10)
write.table(out,
            file="C:/study/bank/submit/submit_train_20171104_ens_lg_rf_xgb_ens_tree_lg_xgb_rf_23_1.csv",
            quote=F, sep=",", row.names=F, col.names=F)
