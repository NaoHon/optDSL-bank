...
for(j in 1:K){
  ...
  #構築データでモデル構築(ロジスティック回帰)
  logi_tmp<-glm(y ~ job + marital + education + housing + loan + 
                  contact + day + month + duration + campaign + poutcome, data=train_tmp, family=binomial(link="logit"))
...
write.table(out,
            file="C:/study/bank/submit/submit_train_20171104_ens_lg_rf_xgb_ens_tree_lg_xgb_19.csv",
            quote=F, sep=",", row.names=F, col.names=F)
