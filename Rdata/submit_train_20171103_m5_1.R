...
#重み付け検証
auc<-roc(train_test$y, (0.5*logi_pred_train_test
                        +4.5*rf_pred_train_test
                        +5.5*xgb_pred_train_test
                        +0.5*stack_pred_train_test)/10)
print(auc)
auc$auc
...
#submitの形式で出力(CSV)
#データ加工
out<-data.frame(test$id, (3*pred_rf+6.5*pred_xgb+0.5*pred_ens_tree_logi)/10)
write.table(out,
            file="C:/study/bank/submit/submit_train_20171103_m5_1.csv",
            quote=F, sep=",", row.names=F, col.names=F)
