...
#submitの形式で出力(CSV)
#データ加工
out<-data.frame(test$id, (3*pred_rf+5.5*pred_xgb+1.5*stack_pred_test)/10)
write.table(out,
            file="C:/study/bank/submit/submit_train_20171104_ens_lg_rf_xgb_ens_tree_lg_xgb_12.csv",
            quote=F, sep=",", row.names=F, col.names=F)
