...
#submitの形式で出力(CSV)
#データ加工
out<-data.frame(test$id, (2.5*pred_rf+6.0*pred_xgb+1.5*stack_pred_test)/10)
write.table(out,
            file="C:/study/bank/submit/submit_train_20171103_ens_lg_rf_xgb_ens_tree_lg_xgb_1.csv",
            quote=F, sep=",", row.names=F, col.names=F)
