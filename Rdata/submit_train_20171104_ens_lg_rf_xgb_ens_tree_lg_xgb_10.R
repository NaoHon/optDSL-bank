...
rf<-randomForest(rf_train_train, #学習データ(説明変数)
                 ...
                 nodesize=80, #生成する各決定木のノードが含むサンプル最小数
...
#submitの形式で出力(CSV)
#データ加工
out<-data.frame(test$id, (3.5*pred_rf+6*pred_xgb+0.5*stack_pred_test)/10)
write.table(out,
            file="C:/study/bank/submit/submit_train_20171104_ens_lg_rf_xgb_ens_tree_lg_xgb_10.csv",
            quote=F, sep=",", row.names=F, col.names=F)
