...
while (lw <= 7) {
  while (rw <= 7) {
    while (xw <= 7) {
      while (sw <= 7) {
        if((lw+rw+xw+sw)==10){
          auc<-roc(train_test$y, (lw*bag_pred_logi_train_test
...
#submitの形式で出力(CSV)
  #データ加工
  out<-data.frame(test$id, (2.5*pred_rf+6.0*pred_xgb+1.5*bag_pred_logi)/10)
  write.table(out,
              file="C:/study/bank/submit/submit_train_20171104_ens_bag_rf_xgb_ens_tree_lg_xgb_1.csv",
              quote=F, sep=",", row.names=F, col.names=F)
